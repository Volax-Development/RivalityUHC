...
public class GAutoLeafDelay implements Listener {
    @EventHandler
    public void breakWood(BlockBreakEvent event){
        if(GConfig.FastBreakTree()){
            ...
            if(mat == Material.LOG || mat == Material.LOG_2) {
                ...
                }.runTaskTimer(GMain.getInstance(), 1, 1);
            }
        }
    }
    ...
    private void checkLeaves(Block block) {
        ...
        Bukkit.getServer().getScheduler().runTask(GMain.getInstance(), new Runnable() {
            @Override
            public void run() {
